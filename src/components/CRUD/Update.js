...
const Update = () => {
  ...
  const { id } = useParams(); // URL 파라미터에서 빌드 ID 가져오기
  ...
  // --- [Update 기능] 기존 데이터 불러오기 ---
  useEffect(() => {
    const savedList = JSON.parse(localStorage.getItem('myBuilds')) || [];
    // URL의 id와 일치하는 빌드 찾기 (타입 변환 주의)
    const targetBuild = savedList.find(item => String(item.id) === String(id));

    if (targetBuild) {
      setFormData(targetBuild);
    } else {
      alert("존재하지 않는 빌드입니다.");
      navigate('/myinfo');
    }
  ...
  // --- [Update 기능] 챔피언 이름 검색창 동기화 ---
  // champions 데이터가 로드되고 formData가 세팅되면 검색창에 이름을 채워줌
  useEffect(() => {
    if (formData.championId && champions.length > 0) {
      const foundChamp = champions.find(c => c.id === formData.championId);
      if (foundChamp) {
        setChampSearch(foundChamp.name);
      }
    }
  }, [formData.championId, champions]);

  // --- 초기 데이터 로딩 (DataDragon) ---
  useEffect(() => {
  ...
  useEffect(() => {
    if (formData.championId && version) {
      const fetchSkin = async () => {
        ...
        // Update 시에는 기존 skinId를 유지해야 하므로 초기화 로직 제거 또는 조건부 처리
        // 만약 챔피언을 바꿨을 때만 초기화하고 싶다면 이전 값을 비교해야 함.
        // 여기서는 간단히: 로드된 detail이 현재 formData와 다를 수 있으나, 
        // skinId는 formData에 이미 저장되어 있으므로 굳이 '0'으로 리셋하지 않음.
      };
      fetchSkin();
    }
  ...
  const selectChampion = (champ) => {
    setFormData({ ...formData, championId: champ.id, skinId: '0' }); // 챔피언 변경 시 스킨 초기화
  ...
  const handleUpdate = () => {
    ...
    // ID가 일치하는 항목을 찾아 교체 (map 사용)
    const updatedList = savedList.map(item => 
      String(item.id) === String(id) ? { ...formData, id: Number(id), version } : item
    );
                        ...
                        <div className="p-2 border border-secondary rounded bg-black bg-opacity-25">
                          ...
                          <div className="d-flex flex-wrap gap-2 justify-content-center">
                            {spells.map(s => {
                               ...
                               return (
                                 <img key={s.id} src={`https://ddragon.leagueoflegends.com/cdn/${version}/img/spell/${s.id}.png`} alt={s.name} title={s.name}
               ...
               <div className="row g-4">
                 <div className="col-md-6 border-end border-secondary pe-4">
                   ...
                   <div className="d-flex gap-3 mb-4 justify-content-center">
                     {runes.map(r => (
                       <img key={r.id} src={`https://ddragon.leagueoflegends.com/cdn/img/${r.icon}`} alt={r.name} title={r.name}
                     ...
                     <div className="d-flex flex-column gap-4 align-items-center">
                       <div className="d-flex gap-4 p-2 bg-black bg-opacity-25 rounded-pill">
                         {selectedRuneStyleObj.slots[0].runes.map(k => (
                           <img key={k.id} src={`https://ddragon.leagueoflegends.com/cdn/img/${k.icon}`} alt={k.name} title={k.name}
                       ...
                       {selectedRuneStyleObj.slots.slice(1).map((slot, idx) => {
                         ...
                         return (
                           <div key={idx} className="d-flex gap-4">
                             {slot.runes.map(rune => (
                               <img key={rune.id} src={`https://ddragon.leagueoflegends.com/cdn/img/${rune.icon}`} alt={rune.name} title={rune.name}
